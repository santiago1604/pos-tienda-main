...
use App\Models\User;
use App\Mail\RepairEntregadaMail;
...
class RepairController extends Controller
{
    ...
    public function update(Request $r, Repair $repair)
    {
        ...
        if (isset($data['status']) && $data['status'] === 'delivered') {
            ...
            // Enviar correo de notificación al cliente si proporcionó su email
            if (!empty($repair->customer_email)) {
                try {
                    Mail::to($repair->customer_email)->send(new RepairEntregadaMail($repair));
                } catch (\Throwable $e) {
                    // El correo es informativo; no bloqueamos el flujo si falla
                    \Illuminate\Support\Facades\Log::warning('No se pudo enviar correo de reparación entregada: ' . $e->getMessage());
                }
            }
        }
